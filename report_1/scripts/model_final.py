...
def run_ppi(I0, T, A=None, alpha=.1, phi=.5, tau=.5, R=None, 
            gov_func=None, P0=None, H0=None, PF=None, pf=1, RD=None, b=None, tolerance=1e-3):
    ...
    ----------
        ...
        b: float
            A factor for the overall increase or shrinkage of the total budget.
            It should only be used in a counterfactual analysis where the baseline
            estimation has a b=1, so if b=0.9, it means that the overall budget
            for transformative public policies shrinked 10% with respect to the
            baseline estimation.
...
#        else:
#            cnorm = np.zeros(N) # initialize a zero-vector to store the normalized contributions
#            cnorm[R] = C/np.max([P.max(), 1-b]) # compute normalized contributions only for instrumental nodes
#            gammas = (alpha + cnorm)/(alpha + np.exp(-S/(np.mean(gaps/gaps0)))) # compute probability of succesful growth            
