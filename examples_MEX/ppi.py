...
def run_ppi(I0, T, A=None, alpha=.1, phi=.5, tau=.5, R=None, 
    ...
    C = np.random.rand(n).astype(float)*P # vector of contributions
    ...
    X = np.random.rand(n).astype(float)-.5 # vector of actions
    Xt = np.random.rand(n).astype(float)-.5 # vector of lagged actions
